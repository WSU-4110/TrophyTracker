 ...
 */
export default async function edit(commentId: string, content: string): Promise<ActionsResponse> {
    try {
        ...
        if (comment.author._id != session.user.person._id) throw new Error("You can only edit your own comments");
